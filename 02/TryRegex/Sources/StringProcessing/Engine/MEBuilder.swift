...
extension MEProgram.Builder {
  mutating func buildMoveImmediate(
    ...
    instructions.append(
      .init(.moveImmediate, .init(immediate: uint, int: into))
    )
  }
  ...
  mutating func buildCondBranch(
    to t: AddressToken, ifZeroElseDecrement i: Int
  ) {
    instructions.append(.init(.condBranchZeroElseDecrement, .init(int: i)))
    fixup(to: t)
  }
